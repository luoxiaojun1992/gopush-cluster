...
// StartMQTT start mqtt listen.
func StartMQTT() error {
	for _, bind := range Conf.WebsocketBind {
		log.Info("start mqtt listen addr:\"%s\"", bind)
		go mqttListen(bind)
	}
...
func (this *Server) handleConnection(c io.Closer) (svc *mqtt.service, err error) {
	if c == nil {
...
func (this *Server) getSession(svc *mqtt.service, req *message.ConnectMessage, resp *message.ConnackMessage) error {
